...
- Any `backdrop-blur`/`filter`/`transform` on an ancestor creates a CSS containing block for `position: fixed` descendants — a `fixed inset-0` overlay nested inside such an element gets trapped inside *that element's* box instead of the viewport. Render viewport-covering overlays as siblings of the blurred/transformed element, not descendants.
...
- `src/components/Header.tsx` — pins/reshapes the nav on scroll; branches on `window.matchMedia("(max-width: 40em)")`, evaluated once at mount (not reactive to resize — this matches the original behavior, not a bug to "fix").
  - Its mobile menu is a full-screen `fixed inset-0` overlay rendered as a **sibling** of `<header>`, not nested inside it (see the backdrop-blur/containing-block gotcha above — `<header>` itself has `backdrop-blur-md`). It also locks `document.body.style.overflow` while open, restored on close.
...
**Forms:** `src/sections/Contact.tsx` posts via plain `<form action="https://formspree.io/f/..." method="POST">` — no fetch/JS, Formspree handles delivery server-side. Follow the same no-JS pattern for any future form.
