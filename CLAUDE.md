...
- `src/sections/Services.tsx` — the most complex animation: a pinned section (plain `ScrollTrigger.create()`, deliberately *not* a shared `gsap.timeline()`) that reveals three repeated blocks via a `revealRefs` ref-array + `addToRefs` callback, targeting `el.childNodes[0/1/2]` positionally. This positional-childNodes approach is fragile against markup changes — if you reorder or wrap the JSX children inside a `Content` block, the animation targets will silently break.
  - Don't wrap these reveal tweens back into a `gsap.timeline({ scrollTrigger: {...} })`. A tween that has its own `scrollTrigger` gets `.pause()`d and driven directly by that ScrollTrigger, not by the parent timeline's playhead — so the timeline's sequencing becomes dead weight while still adding a redundant ScrollTrigger instance per tween (verified against GSAP source; this was the root cause of a fast-scroll jank bug here).
  - All scroll-scrubbed tweens use `scrub: <number>` (e.g. `0.4`) + `fastScrollEnd: true`, never bare `scrub: true` — the unsmoothed boolean caused visible jank on fast wheel-scroll given the ~10 ScrollTriggers running concurrently under this one pinned section. Keep this pattern for any new scroll-scrubbed animation added here.
  - The `ref` div (black backdrop, `bg-black`) is pinned via `pinReparent` for the *entire* section scroll, not just briefly at the top — it stays visible behind all three `TextBlock` instances the whole time. Any text color in `TextBlock`/these blocks must assume a black background, not the page's default pale `--color-background`, even though the blocks sit in normal document flow (a `text-nav2` subtext here was invisible against it until fixed).
