...
- `src/sections/Services.tsx` — the most complex animation: a pinned GSAP timeline that reveals three repeated blocks via a `revealRefs` ref-array + `addToRefs` callback, targeting `el.childNodes[0/1/2]` positionally. This positional-childNodes approach is fragile against markup changes — if you reorder or wrap the JSX children inside a `Content` block, the animation targets will silently break.
